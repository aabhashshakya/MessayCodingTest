...
あなたのミッションは、このプロジェクトをベースに、弊社独自の目と顔のトラッキングライブラリ 「Messay Development Kit (MDK)」 を使用したデモ画面を実装することです。
...
1. **Messay Development Kit (MDK) の組み込み**:
   `composeApp` モジュール内でMDKへの依存関係を適切に追加してください。

2. **インタラクティブなUI**:
   `MdkTarget.EyeCloseHold`と`MdkTarget.FaceMovement`を使って、目を一定時間とじてから開くことで発火する操作と、顔の動きを使ったポインターの操作を、１つの画面（`App.kt`）に実装してください。
   画面の内容は、自由に考えてください。ゲームでも、ツールでもなんでも構いません。ユーザーが使いやすいようなUI/UXとなるよう、各自工夫してみてください。
   実装参考用の画面が`Sample.kt`にあるので、こちらも参考にしていただければと思います。
...
インカメラから取得した画像から、目の開閉や顔の角度、虹彩の位置などを検出し、ユーザーインタラクションに適したデータ形式に変換する、弊社独自の[SDK](https://messay.ndk-group.co.jp/en/sdk/)ライブラリです。
...
- **機能性**:要件通りに目と顔のトラッキングが動作しているか。

- **設計力**: KMPの特性（共通化）を理解し、適切なアーキテクチャで実装されているか。

- **UI/UX**: 目や顔のジェスチャーに対し、ユーザーにフィードバック（視覚効果など）を適切に返せているか。
...
2. 課題完了後、GitHub等のリポジトリのURL、またはZIPファイルを共有してください。質問やご意見がございましたら、**この**リポジトリにIssueを作成し、投稿してください。

3. `SUBMISSION.md`（任意）を作成し、アピールポイントや実装上の工夫があれば記載してください。 

あなたのコードを拝見するのを楽しみにしています！🚀